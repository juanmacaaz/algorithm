...
def compute(M):
	M = bimatrix(M)
	treeGenratorGraf = minimum_spanning_tree(csr_matrix(M))
	
	MST = _csr_gen_triples(treeGenratorGraf)
	print("Arestas arbol generador:")
	[print(chr(x[0]+97).upper()+chr(x[1]+97).upper(), end=',') for x in sorted(MST)]
	odd_vertices = _odd_vertices_of_MST(MST, csr_matrix(M).shape[0])
	print("\nCoste minimo arbol generador:")
	print(sum(treeGenratorGraf.data))
	print("Vertices escogidos:")
	[print(chr(x+97).upper(), end='') for x in sorted(odd_vertices)]
	print("\nFormas de aparellar:")
	bipartite_set = [set(i) for i in itertools.combinations(set(odd_vertices), int(len(odd_vertices)/2))]
	print(len(bipartite_set)/2)
	bipartite_graphs = bipartite_Graph(M, bipartite_set, odd_vertices)
	indexes = min_Munkres(M, bipartite_graphs)
	print("Arestas a duplicar:")
	[print(chr(x[0]+97).upper()+chr(x[1]+97).upper(), end=',') for x in sorted(indexes)]
	print("\nPeso a duplicar:")
	print(sum( (M[x[0]][x[1]]) for x in indexes))

	GI = nx.MultiGraph()
	GI.add_edges_from(sorted(MST))
	GI.add_edges_from(sorted(indexes))
	eulerian = list(nx.eulerian_circuit(GI, source=0, keys=True))
	
	newList = []
	for x in eulerian:
		newList.append(x[0])
	newList.append(eulerian[0][0])

	newList = newList[::-1]
	print("Circuito euraliano:")
	[print(chr(x+97).upper(), end='') for x in newList]
	print()
	print("Circuito hamiltonia:")
	listaFinal = []
	for x in newList:
		if x not in listaFinal:
			listaFinal.append(x)
	listaFinal.append(eulerian[0][0])

	
	[print(chr(x+97).upper(), end='') for x in listaFinal]
	verticesParaLaSuma = []

	for x in range(len(listaFinal)-1):
		verticesParaLaSuma.append([listaFinal[x],listaFinal[x+1]])
	
	sumaTotal = 0
	for x in verticesParaLaSuma:
		sumaTotal+=M[x[0]][x[1]]
	print()
	print("Coste anterior:")
	print(sumaTotal)
	print("Coste final:")
	print((sumaTotal*2)/3)